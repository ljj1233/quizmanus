...
| ------------------------------------------------------ | -------- | ---- | ----- | ---- |
| Qwen-2.5-7B-Instruct                                   | 53.12   | 74.2 | 77.09 |   4.04   |
...
2. 一道一道题目生成会使得在后续token消耗变得超级大；虽然已经在主流程中引入了指纹/状态压缩来缩短上下文，仍需要在长批次生成里验证效果。
3. 爬虫已经改为使用本地 Trafilatura 获取网页正文，避免第三方额度限制，但在极端站点上仍需要监控抓取失败与兼容性。

## 未来研发方向
1. 不止步于基于本低知识库的试卷生成系统，而是支持用户实时上传pdf，实时根据给定pdf生成题目、试卷。
2. 加入问答系统，给用户解答问题。
3. 持续优化指纹提取的准确性和滑动窗口状态压缩策略，保证长对话下的稳定性与去重效果。
4. 目前仅仅支持纯文本且题目token量没那么大的生物、地理、历史、政治，未来可以加上语文、英语、数学等。
